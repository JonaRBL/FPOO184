...
class lt1():
    def __init__(self):
        self.edLt1 = Tk()
        self.edLt1.title("LT1")
        self.edLt1.geometry("600x400")

        #2.Agregamos los Frames
        self.seccion1 = Frame(self.edLt1, bg = "red")
        self.seccion1.pack(expand = True, fill = 'both')

        self.titulo = Label (self.seccion1, text = "LT1", bg = "White" ,width=50)
        ...
        canv.create_image(1,1,anchor=NW, image=self.imag)#20, 20, anchor=NW, image=img)
        ...
        self.edLt1.config(menu=self.menubar)
        self.file_menu = Menu(self.menubar, tearoff=False)
        self.file_menu.add_command( label="Edificio A", command=CambiarVentana1)
        self.file_menu.add_command( label="Edificio B", command=CambiarVentana2)
        self.file_menu.add_command( label="Edificio C", command=CambiarVentana3)
